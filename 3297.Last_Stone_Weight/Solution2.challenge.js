...
var lastStoneWeight = function(stones) {
    ...
    const maxHeap = MaxHeap.heapify(stones);

    while (maxHeap.size() > 1) {
        const firstMax = maxHeap.extractRoot();
        const secondMax = maxHeap.extractRoot();

        const diff = firstMax.getKey() - secondMax.getKey();
        if (diff > 0) {
            maxHeap.insert(diff);
        }
    }

    return maxHeap.size() === 0 ? 0 : maxHeap.root().getKey();
    // Solution 1
    // Sort NlogN
